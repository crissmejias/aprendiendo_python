...
def listar_tareas(
    orden=None, direccion=None, completada=None, fecha_inicio=None, fecha_fin=None
):
    conn = None
    try:
...
def traer_tarea(id):
    ...
    try:
        conn, cursor = conectar_a_bd("testing_tareas", False)
        sql = (
            """SELECT id, titulo, completada,fecha_creacion FROM tareas WHERE id = %s"""
        )
...
def editar_tarea(id, respuesta):
    ...
    try:
        ...
        if cursor.rowcount == 0:
            return None
        id, titulo, completada = cursor.fetchone()
        tarea_editada = {"id": id, "titulo": titulo, "completada": completada}
        conn.commit()
